...
- To predict character survival using a Random Forest classifier and assess feature importance.
...
- **Multiple Linear Regression**: A comprehensive model incorporating multiple features to predict total appearances.
...
The analysis revealed that nobility status is a significant predictor of a character's total appearances, with noble characters appearing more frequently than commoners. The multiple regression model highlighted the importance of appearances in specific books. The Random Forest classifier achieved a 70.1% accuracy in predicting survival, with certain features being particularly influential.
...
To replicate the analysis, clone the repository, install the required packages, and run the Jupyter notebook.
